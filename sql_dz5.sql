...
INSERT INTO user_info (user_id, gender, birthday, photo_id, hometown) VALUES 
...
SELECT firstname, lastname, user_info.hometown AS city, user_info.gender 
FROM users 
JOIN user_info ON users.id = user_info.user_id 
WHERE YEAR(CURRENT_DATE()) - YEAR(user_info.birthday) <= 20;

select * from users_info;


SELECT u.firstname, u.lastname, count(m.id) as messages_count,
...
SELECT body, created_at, 
       LEAD(created_at) OVER (ORDER BY created_at ASC) - created_at as time_diff
FROM messages
